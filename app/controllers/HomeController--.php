...
class HomeController extends BaseController
{
	...
	public function getRetos()
	{
		...
		$fechaInicio = strtotime('2016-02-01 09:00:00');
		$fechaFinal = strtotime('2016-03-03 21:00:00');
	...
	public function getGanadores()
	{
		...
		//datos de "ultra fan"
		$datosUltraFan = array(
			'ultimos'	=> Ganadoresultrafan::select(array('nombre', DB::raw("DATE(created_at) AS fecha")))->whereDate('created_at', '=', '2016-02-15')->get()->toArray(),
			'todos'		=> Ganadoresultrafan::select(array('nombre', DB::raw("DATE(created_at) AS fecha")))->get()->toArray()
		);

		//datos de "tribuna entel"
		$ganadoresTribunaEntelParrafo1 = Ganadorestribunaentel::select(array('nombre', DB::raw("DATE(created_at) AS fecha")))->whereDate('created_at', '=', '2016-02-17')->take(10)->get()->toArray();
		$ganadoresTribunaEntelParrafo2 = Ganadorestribunaentel::select(array('nombre', DB::raw("DATE(created_at) AS fecha")))->whereDate('created_at', '=', '2016-02-17')->skip(10)->take(10)->get()->toArray();
		...
		$fechaAyer = strtotime($fechaUltimoSorteo . ' 12:00:00');
		$fechaInicio = strtotime('2016-02-01 12:00:00');
		$htmlCalendario = '';
